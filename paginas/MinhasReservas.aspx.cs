...
{
    public partial class MinhasReservas : BasePage
    {
        ...
        private void CarregarReservas(int clienteId)
        {
            try
            {
                ...
                // Usar a URL atual da requisi��o para garantir que a porta esteja correta
                var baseUrl = Request.Url.GetLeftPart(UriPartial.Authority);

                if (reservas == null || reservas.Count == 0)
                {
                    reservasContainer.InnerHtml = "<div class='alert alert-info'><i class='fas fa-info-circle'></i> Voc� ainda n�o possui reservas. <a href='Default.aspx'>Fa�a sua primeira reserva!</a></div>";
                    return;
                }
                ...
                foreach (var reserva in reservas)
                {
                    ...
                    {
                        previsaoHtml = string.Format("<p><strong>Previs�o de Entrega:</strong> {0}</p>",
                    ...
                    {
                        observacoesHtml = string.Format("<p><strong>Observa��es:</strong> {0}</p>", reserva.Observacoes);
                    }

                    // Verificar se TokenAcesso existe, se n�o, gerar um
                    string token = reserva.TokenAcesso;
                    if (string.IsNullOrEmpty(token))
                    {
                        // Se n�o tem token, usar o ID da reserva como fallback (n�o ideal, mas funcional)
                        token = reserva.Id.ToString();
                    }
                    
                    // Usar URL relativa para o link (evita problemas com porta/dom�nio)
                    string linkReserva = ResolveUrl("~/paginas/VerReserva.aspx?token=" + System.Web.HttpUtility.UrlEncode(token));
                    
                    // URL completa para compartilhamento (precisa ser absoluta)
                    string linkReservaCompleto = baseUrl.TrimEnd('/') + "/paginas/VerReserva.aspx?token=" + System.Web.HttpUtility.UrlEncode(token);
                    ...
                    // Bot�es de compartilhamento (apenas se n�o estiver cancelada)
                    ...
                    var reservaDetalhes = new
                    {
                        ...
                        email = !string.IsNullOrEmpty(reserva.Email) ? reserva.Email : "N�o informado",
                        telefone = !string.IsNullOrEmpty(reserva.Telefone) ? reserva.Telefone : "N�o informado",
                        observacoes = !string.IsNullOrEmpty(reserva.Observacoes) ? reserva.Observacoes : "Nenhuma observa��o",
                        previsaoEntrega = reserva.PrevisaoEntrega.HasValue ? reserva.PrevisaoEntrega.Value.ToString("dd/MM/yyyy HH:mm") : "N�o definida",
                    ...
                    html += string.Format(@"
                        ...
                        "", // {11} - removido (era disabled do bot�o excluir)
                        textoCompartilhar.Replace("\"", "&quot;"), // {12}
                        linkReservaCompleto, // {13} - URL completa para compartilhamento
                        botaoCancelar, // {14} - Bot�o de cancelar
                        botaoExcluir, // {15} - Bot�o de excluir
                        botoesCompartilharHtml, // {16} - Bot�es de compartilhamento (vazio se cancelada)
        ...
        private void CancelarReserva(int reservaId, int clienteId)
        {
            try
            {
                ...
                // Verificar se a reserva j� est� cancelada (verificando status "Cancelado")
                var statusCancelado = _databaseService.ObterStatusReservaPorNome("Cancelado");
                ...
                {
                    string motivo = "N�o foi poss�vel cancelar a reserva.";
                    if (reserva != null)
                    {
                        if (jaCancelada)
                        {
                            motivo = "Esta reserva j� foi cancelada.";
                        }
                        else if (reserva.StatusId.HasValue && !_databaseService.StatusPermiteExclusao(reserva.StatusId.Value))
                        {
                            motivo = "Esta reserva n�o pode ser cancelada pois j� est� em " + reserva.Status + ".";
                        }
                    }
        ...
        private void ExcluirReserva(int reservaId, int clienteId)
        {
            try
            {
                ...
                if (podeExcluir)
                {
                    ...
                    // Enviar email ANTES de excluir (pois depois n�o teremos mais os dados)
                    System.Threading.Tasks.Task.Run(() =>
                    {
                        try
                        {
                    ...
                    MostrarAlerta("Reserva exclu�da com sucesso!", "success");
                    CarregarReservas(clienteId);
                }
                else
                {
                    string motivo = "N�o foi poss�vel excluir a reserva.";
                    if (reserva != null)
                    {
                        if (reserva.StatusId.HasValue && !_databaseService.StatusPermiteExclusao(reserva.StatusId.Value))
                        {
                            motivo = "Esta reserva n�o pode ser exclu�da pois j� est� em " + reserva.Status + ".";
                        }
