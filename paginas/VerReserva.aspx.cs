...
{
    public partial class VerReserva : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            // Verificar se � uma requisi��o de atualiza��o
            ...
            {
                MostrarErro("Token de acesso n�o fornecido.");
                return;
            }
            ...
            try
            {
                ...
                if (reserva == null)
                {
                    MostrarErro("Reserva n�o encontrada ou token inv�lido.");
                    return;
                }

                // Verificar se cliente est� logado e fazer login autom�tico se necess�rio
        ...
        {
            // Se a reserva tem cliente associado e n�o est� logado, fazer login autom�tico
            if (reserva.ClienteId.HasValue && Session["ClienteId"] == null)
            {
                // Usar ClienteId diretamente ao inv�s de buscar por email
        ...
        private void ExibirReserva(Models.Reserva reserva)
        {
            ...
            // Itens da reserva (edit�veis ou n�o)
            string itensHtml = "";
            if (podeEditar)
            {
                // Interface edit�vel
                ...
                {
                    // Verificar se � um saco promocional (tem produtos dentro)
            ...
            {
                previsaoHtml = string.Format("<div class='alert alert-info'><i class='fas fa-calendar'></i> <strong>Previs�o de Entrega:</strong> {0}</div>",
                    reserva.PrevisaoEntrega.Value.ToString("dd/MM/yyyy"));
            }

            // Formul�rio de edi��o ou exibi��o de observa��es
            string observacoesHtml = "";
            if (podeEditar)
            {
                observacoesHtml = string.Format(@"
                    <div class='mb-3'>
                        <label class='form-label'><strong>Observa��es:</strong></label>
            ...
            else
            {
                ...
                {
                    observacoesHtml = string.Format("<div class='alert alert-secondary'><strong>Observa��es:</strong> {0}</div>", 
                        System.Web.HttpUtility.HtmlEncode(reserva.Observacoes));
                }
            }
            ...
            // Verificar se a reserva est� cancelada (verificando status "Cancelado")
            ...
            // Se��o de compartilhamento (apenas se n�o estiver cancelada)
            ...
            // Bot�es de edi��o (apenas se pode editar)
            ...
            if (podeEditar)
            {
                botoesEdicaoHtml = string.Format(@"
                    <div class='border-top pt-3 mt-3'>
                        ...
                        <p class='text-muted'>Voc� pode editar a data de retirada, observa��es e itens enquanto a reserva estiver aberta.</p>
                        <div class='mt-3'>
            ...
            {
                botoesEdicaoHtml = "<div class='alert alert-warning mt-3'><i class='fas fa-info-circle'></i> Esta reserva n�o pode mais ser editada ou exclu�da pois j� est� em " + System.Web.HttpUtility.HtmlEncode(reserva.Status) + ".</div>";
            }

            // Data de retirada (edit�vel ou n�o)
            ...
            conteudoContainer.InnerHtml = string.Format(@"
                ...
                <div class='reserva-detalhes'>
                    ...
                    <div class='row mb-4'>
                        <div class='col-md-6'>
                            <h5><i class='fas fa-calendar-alt'></i> Informa��es da Reserva</h5>
                            {3}
                ...
                previsaoHtml,
                !string.IsNullOrEmpty(reserva.Nome) ? System.Web.HttpUtility.HtmlEncode(reserva.Nome) : "N�o informado",
                !string.IsNullOrEmpty(reserva.Email) ? System.Web.HttpUtility.HtmlEncode(reserva.Email) : "N�o informado",
                !string.IsNullOrEmpty(reserva.Telefone) ? System.Web.HttpUtility.HtmlEncode(reserva.Telefone) : "N�o informado",
                itensHtml,
                observacoesHtml,
                linkReserva,
                textoCompartilhar.Replace("\"", "&quot;"),
                ...
                reserva.ValorTotal.ToString("F2"),
                compartilharHtml // {17} - Se��o de compartilhamento (vazia se cancelada)
            );
        }
        ...
        private void SalvarReserva()
        {
            try
            {
                ...
                if (reserva == null)
                {
                    MostrarErro("Reserva n�o encontrada.");
                    return;
                }
                ...
                if (!podeEditar)
                {
                    MostrarErro("Esta reserva n�o pode ser editada.");
                    return;
                }
                ...
                // Buscar todos os campos de itens do formul�rio
                foreach (string key in Request.Form.AllKeys)
                {
                    if (key != null && key.StartsWith("itens[") && key.Contains("].ProdutoId"))
                    {
                        // Extrair �ndice do item
                                    ...
                                    if (!string.IsNullOrEmpty(produtos))
                                    {
                                        ...
                                        if (produtoSaco != null && produtoSaco.EhSacoPromocional && !string.IsNullOrEmpty(produtoSaco.Produtos))
                                        {
                                            try
                                            {
                                                ...
                                                {
                                                    // Se falhar ao parsear, tentar como string separada por v�rgula
                                                ...
                                                foreach (var produtoJson in produtosJson)
                                                {
                                                    ...
                                                    {
                                                        MostrarErro("O produto selecionado n�o � permitido neste saco. Por favor, selecione apenas produtos permitidos.");
                                                        return;
                                                    }
                                                }
                                            }
                ...
                // Se n�o houver itens no formul�rio, manter os itens atuais
                ...
                // Recarregar p�gina com mensagem de sucesso
        ...
        public static object ObterProdutosDisponiveis(string produtosPermitidosJson = null)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(produtosPermitidosJson))
                {
                    ...
                    {
                        // Se falhar ao parsear, retornar todos os produtos n�o-sacos
        ...
        private void MostrarErro(string mensagem)
        {
            conteudoContainer.InnerHtml = string.Format(@"
                <div class='alert alert-danger'>
                    ...
                    <p>{0}</p>
                    <div class='mt-3'>
                        <a href='Default.aspx' class='btn btn-primary'>Voltar para P�gina Inicial</a>
                    </div>
                </div>",
                mensagem
            );
